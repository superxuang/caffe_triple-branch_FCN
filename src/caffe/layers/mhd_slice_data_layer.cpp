	...
	void MHDSliceDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		while (std::getline(infile, line)) {
			...
			if (!contour_labels.empty()) {
				...
				//resample_spacing[2 - extract_direct] = origin_spacing[2 - extract_direct];
				//resample_size[2 - extract_direct] = origin_size[2 - extract_direct];
				//resample_origin[2 - extract_direct] = origin_origin[2 - extract_direct];
