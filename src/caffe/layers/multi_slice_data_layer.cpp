...
namespace caffe {
	...
	void MultiSliceDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
		...
		const int extract_direct = mhd_data_param.extract_direction();
		...
		while (std::getline(infile, line)) {
			...
			if (!contour_labels.empty()) {
				...
				//	resample_spacing[0] = mhd_data_param.max_width() * mhd_data_param.resample_spacing_x() / resample_size[0];
				//else
				...
				//	resample_spacing[1] = mhd_data_param.max_height() * mhd_data_param.resample_spacing_y() / resample_size[1];
				//else
				...
				//	resample_spacing[2] = mhd_data_param.max_length() * mhd_data_param.resample_spacing_z() / resample_size[2];
				//else
				...
				resample_size[2 - extract_direct] = origin_size[2 - extract_direct] * origin_spacing[2 - extract_direct] / resample_spacing[2 - extract_direct];
				resample_origin[2 - extract_direct] = origin_origin[2 - extract_direct] + origin_size[2 - extract_direct] * origin_spacing[2 - extract_direct] * 0.5
					- resample_size[2 - extract_direct] * resample_spacing[2 - extract_direct] * 0.5;
				//////////////////////////////////////////////////////////////////////////
				...
				case 0: // axial
				{
					for (int i = 0; i < resample_size[2]; ++i)
					{
						...
						{
							int ii = (resample_origin[2] + i * resample_spacing[2] - origin_origin[2]) / origin_spacing[2];
				...
				case 1: // coronal
				{
					for (int i = 0; i < resample_size[1]; ++i)
					{
						...
						std::vector<int> labels;
						labels.resize(contour_num_, 0);
						for (int j = 0; j < exist_contours.size(); ++j)
						{
							int ii = (resample_origin[1] + i * resample_spacing[1] - origin_origin[1]) / origin_spacing[1];
				...
				case 2: // sagittal
				{
					for (int i = 0; i < resample_size[0]; ++i)
					{
						...
						for (int j = 0; j < exist_contours.size(); ++j)
						{
							int ii = (resample_origin[0] + i * resample_spacing[0] - origin_origin[0]) / origin_spacing[0];
						...
						for (int j = 0; j < contour_pair_list.key_size(); ++j)
						{
							if (labels[contour_pair_list.key(j)] == 1)
								labels[contour_pair_list.value(j)] = 1;
						}
	...
	void MultiSliceDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
		...
		for (int batch_id = 0; batch_id < batch_size; ++batch_id)
		{
			...
			const ImageSliceType::PointType& slice_origin = center_slice_record->data_->GetOrigin();
