...
namespace caffe {
	...
	template <typename Dtype>
	double ROIAlignLayer<Dtype>::ResampleCubic(double x, double y, double z, const Dtype * pdfValue, int nWidth, int nHeight, int nLength, int pool_index, int* argmax_data, Dtype* w_data)
	{
		Dtype dfCubicValue;
		int i = x;
		int j = y;
		...
		for (int rr = k - 1, ss = 0; rr <= k + 2; rr++, ss++){
			for (int r = j - 1, s = 0; r <= j + 2; r++, s++){
				for (int c = i - 1, t = 0; c <= i + 2; c++, t++){
					...
					temp_rr = min(max(Dtype(rr), Dtype(0)), Dtype(nLength - 1));
					values[ss][s][t] = pdfValue[temp_rr*nHeight*nWidth + temp_r*nWidth + temp_c];
					argmax_data[64 * pool_index + ss * 16 + s * 4 + t] = temp_rr*nHeight*nWidth + temp_r*nWidth + temp_c;
				}
			}
		}
		/*calc the coeff*/
		...
		for (int ss = 0; ss < 4; ss++) {
			for (int s = 0; s < 4; s++) {
				for (int t = 0; t < 4; t++) {
					...
					w_data[64 * pool_index + ss * 16 + s * 4 + t] = A[t] * C[s] * CC[ss];
				}
			}
		}
		return dfCubicValue;
	...
	void ROIAlignLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
		...
		if (bi_type == BiCubic) {
			...
			top_shape[2] = pooled_length_;
			top_shape[3] = pooled_height_;
			top_shape[4] = pooled_width_ * 8;
		}

		bili_idx.Reshape(top_shape);
		bili_w.Reshape(top_shape);
	}
